...
calculate_rsquared_table <- function(model, my_dis_test, my_dis_train) {
  # Predictions for testing and training sets
  yhat <- predict(model, newdata = my_dis_test)
  yhat_train <- predict(model, newdata = my_dis_train)
  
  # Calculate R-squared for testing set
  rsquared_test <- 1 - mean((yhat - my_dis_test$duration_days)^2) / var(my_dis_test$duration_days)
  
  # Calculate R-squared for training set
  rsquared_train <- 1 - mean((yhat_train - my_dis_train$duration_days)^2) / var(my_dis_train$duration_days)
  
  # Calculate adjusted R-squared for testing and training sets
  n <- nrow(my_dis_test) # Number of observations in the test set
  p <- 8 # Number of predictors in the models (adjust as needed)
  adj_rsquared_test <- 1 - (1 - rsquared_test) * (n - 1) / (n - p - 1)
  adj_rsquared_train <- 1 - (1 - rsquared_train) * (n - 1) / (n - p - 1)
  
  # Create a data frame to store the results
  results <- data.frame(Model = c("Regression Tree", "Bagging", "Random Forest", "Boosting"), 
                        R_squared_Test = c(rsquared_test, NA, NA, NA), 
                        R_squared_Train = c(rsquared_train, NA, NA, NA),
                        Adjusted_R_squared_Test = c(adj_rsquared_test, NA, NA, NA),
                        Adjusted_R_squared_Train = c(adj_rsquared_train, NA, NA, NA))
  return(results)
}
...
# Fit Regression Tree
set.seed(1)
tree.dis <- tree(duration_days ~ ., data = my_dis_train)
prune.dis <- prune.tree(tree.dis, best = 5) # Prune the tree to improve performance

# Fit Bagging
set.seed(1)
bag.dis <- randomForest(duration_days ~ ., data = my_dis_train, mtry = ncol(my_dis_train) - 1)

# Fit Random Forest
set.seed(1)
rf.dis <- randomForest(duration_days ~ ., data = my_dis_train)

# Fit Boosting
set.seed(1)
boost.dis <- gbm(duration_days ~ ., data = my_dis_train, distribution = "gaussian", n.trees = 5000, interaction.depth = 1, cv.folds = 5)

# Calculate R-squared and adjusted R-squared for each model
results_tree <- calculate_rsquared_table(prune.dis, my_dis_test, my_dis_train)
results_bagging <- calculate_rsquared_table(bag.dis, my_dis_test, my_dis_train)
results_rf <- calculate_rsquared_table(rf.dis, my_dis_test, my_dis_train)
results_boosting <- calculate_rsquared_table(boost.dis, my_dis_test, my_dis_train)

# Combine the results into a single data frame
all_results <- rbind(results_tree, results_bagging, results_rf, results_boosting)
